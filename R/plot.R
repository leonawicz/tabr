...
#'
#' Create a fretboard diagram ggplot object. Number of strings is derived from
#' \code{tuning}.
#' See \code{\link{tunings}} for pre-defined tunings and examples of explicit
#' tunings.
#' \code{tuning} affects point labels when \code{labels = "notes"}.
#'
#' @param string integer or as \code{tabr}-style character string, string
#' numbers.
...
#' @param mute logical, whether to mute notes, typically a vector corresponding
#' to \code{string} and \code{fret}.
...
fretboard_plot <- function(string, fret, labels = NULL, mute = FALSE,
  ...
  if(0 %in% fret_range){
    ...
    g <- g + ggplot2::geom_segment(
      ...
      if(horizontal & !left_handed){
        ggplot2::scale_y_continuous(limits = fret_range2)
      } else {
    ...
    g <- g + if(horizontal & !left_handed){
      ggplot2::scale_y_continuous(
        limits = fret_range, labels = fret_range[1], breaks = fret_range[1])
      } else {
        ggplot2::scale_y_reverse(
          limits = rev(fret_range), labels = fret_range[1],
          breaks = fret_range[1])
      }
  }
  if(show_tuning){
  ...
  if(any(mute))
    g <- g + ggplot2::geom_point(
      data = d[mute, ], shape = "X", color = d$point_color[mute],
      size = point_size)
  g <- g + ggplot2::geom_point(
