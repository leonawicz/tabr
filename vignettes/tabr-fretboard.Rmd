...
vignette: >
  %\VignetteIndexEntry{Fretboard diagrams plots}
...
`fretboard_plot` is a highly specialized function that takes vector inputs for string numbers and fret numbers and maps the combination element-wise to produce a fretboard diagram. Note that `fretboard_diagram` is a developmental function and the interface and arguments it provides may change.
...
How accidentals are displayed when `labels = "notes"`, as sharps or flats, can be controlled by setting the `key` to a specific key or simply to `sharp` or `flat`. Here, the key of F contains a flat, which alters the labeling from the earlier diagram. 
