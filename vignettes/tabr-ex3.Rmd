...
```{r g1v1_2}
p1a <- p(glue(x1, x2), glue(notate("8", txt), "8*31"), dup(s1, 4))
p1 <- p(glue(x1, x2), 8, dup(s1, 4))
p2 <- volta(p("e4 g b g b4 b g b a4 g b g b4 b g b", 8, dup(s1, 2)), 3)

e2 <- p("e5 g b g f#5 b g b g5 g b g a5 b g b", 8, dup(s1, 2))
e <- list(p(x2, 8, dup(s1, 2)), e2)
p3 <- glue(p1, p1, volta(p(glue(x1, x2, x1), 8, dup(s1, 6)), 1, e))
...
p4 <- glue(
  p(glue(
    ...
    glue(s1, s2, s3, s2, s3, s2, s3, "x 3 1 3 1 3 1 3")),
  ...
  p("s g b g b4 b g b e5 g b g b4 b g b s g b g c5 b g b g5 g b g e5 b g b s g b g e5 b g b b4 g b g g4 g f#4 g", 8,
    glue(s3, s1, s3, s1, s3, "1 3 2 3 1 3 1 3"))
)

f <- function(x) p(glue(x, "g b g", x, "b g b s g b g", x, "b g b"), 8, glue(s1, s3))
p5 <- glue(
  p1,
  p("e4 g b g b4 b g b c5 g b g b4 b g b e4 g b g g4 b g b b4 g c5 b g4 g b4 b", 8, glue(s1, s1, s1, "1 3 1 2 1 3 1 2")),
  p("e4 g b g b4 b g b4 c5 g b g c5 b g b b4 g b g b4 b g b a4 g b g b4 b g b", 8, glue("1 3 2 3 1 2 3 1", s1, s1, s1)),
  p(x1, 8, dup(s1, 2)),
  p("c5 g b g b4 b g b s g b g b4 b g b", 8, glue(s1, s3)),
  f("c5"), f("e5"), f("g5")
)
```
...
```{r g1v1_4}
p6 <- glue(volta(p1), p1, p(x1, 8, dup(s1, 2)), e2)
p7 <- glue(
  p(tp(glue(gsub("e4|g4", "b4", x1), x2), 12, "em"), 8, dup(s1, 8)),
  p(tp(glue(x1, x2), 12, "em"), 8, dup(s1, 4))
)
p8 <- glue(p1, p1)
...
x2 <- gsub("b4", "g4", x1)
x3 <- gsub("a4", "ba4", x2)
p9 <- p(glue(x1, x2, x1, x3), 8, glue(dup("1 3 2 3 21 3 2 3", 7), "21 3 2 3 21 3 2 3"))
p10 <- glue(volta(p1), dup(p1, 2), p("e4", 1, 1))
```
...
```{r g1v1_5}
track1a <- track(glue(p1a, p2, p3, p4, p5, p6, p7, p8, p9, p10))
```
...
p6 <- glue(volta(p("e2*4", 1, 6)), p("e e d d c c a2 a2", 1, "5 5 4 4 5*4"))
p7 <- p("e2 s*7", 1, "6 x*7")

x3 <- p("e2 e2 d d c c e2 g2 f#2", "1*7 2 2", "6 6 4 4 5 5 6*3")
p8 <- x3
p9 <- x3
p10 <- glue(volta(p("e*4", 1, 5)), x1, p("e2", 1, 6))
```
...
```{r g1v2_2}
track1b <- track(glue(p1, p2, p3, p4, p5, p6, p7, p8, p9, p10), voice = 2)
```

## Guitar 2

The second guitar adds a strummed rhythm section. It has only one voice. Again, some additional note info and string number building blocks are helpful for avoiding some code duplication.

```{r g2_1}
i <- "4 8 8"
i1 <- dup(i, 4)
i2 <- glue(i, i, "8*4", i)
i3 <- glue(i, i, "8*4 8- 8*3")
i4 <- glue(i, i, i, "8 8 4")
...
p1b <- sfp(s1, "55000*3 55003*3 55005*4 55003*3", i2)
p1c <- sfp("5s*11 1 3s 3s", "35450*3 35453*3 35455*5 7 007*2", i3)
p1d <- sfp(s2, "022000*3 022003*3 30005*3 20233*3", i4)
p1d2 <- sfp(s2, "022000*3 022002*3 20233*3 30005*3", i1)

p2 <- glue(p1a, p1b, p1c, p1d)
p3 <- volta(glue(p1a, p1b, p1c), 1, list(p1d, p1d2), TRUE)

p4 <- glue(sfp("5s*12", "75000*12", i1), sfp(s1, "55000*13", i2), sfp(s1, "35450*13", i2), sfp(s2, "022000*6 30005*3 20233*3", i4))
...
p6 <- dup(sfp("5s*12", "75000*12", i1), 3)
p7 <- dup(sfp("6s*12", "022000*12", i1), 4)
p8 <- volta(glue(p1a, p1a2), silent = TRUE)
p9 <- glue(p1a, p1b, p1c, p1d2)

p10 <- sfp("5s~ 5421", "75000~ 7500", "1 1") # hack around bug (double pitch full measure tie), again below (p11)
p10a <- dup(p10, 4)
p10b <- volta(dup(p10, 2), silent = TRUE)
p11 <- sfp("5s~ 5421 5s~ 5421 5s~ 5432 5s 64321*2 6s", "75000~ 7500 55000~ 5500 35450~ 3545 75000 30005 20233 022000", "1*7 2 2 1")
```

Create the track from a sequence of phrases.

```{r g2_3}
track2 <- track(glue(p1a, p1a2, p0, p2, p3, p4, p4, p1a, p1a2, p5, p6, p7, p8, p9, p10a, p2, p2, p10b, p11))
```
...
```{r chords1}
chord_names <- c("e:m", "e:m7/d", "c:maj7", "c:maj9", "e:m", "g:5.9", "g:5.9/f#", "e:m5.9", "s")
chord_positions <- c("x75ooo", "x55ooo", "x3545o", "x35435", "o22ooo", "3xooo5", "2xo233", "o22oo2", NA)
