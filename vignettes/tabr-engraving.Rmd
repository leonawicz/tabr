...
This time, when rendering the song, use settings for the three arguments discussed above. Given what is written, it doesn't make sense to change them all. For example, this won't fit well as a waltz (time = "3/4"). But for illustration purposes, pretend the song is actually in the key of D minor (F major). This key has one flat, B, so this will force the B note in the G chord to be annotated as an accidental. Change the time to `8/8` just for the sake of changing it, which won't be any different from `4/4`. Finally, change the tempo to `4 = 120`, which is also equivalent to the default, but will show up in the output slightly differently.

```{r metadata, results="hide", eval=FALSE}
lilypond(song, "ex32.ly", "dm", "8/8", "4 = 120")
tab(song, "ex32.pdf", "dm", "8/8", "4 = 120")
```
