...
The previous example used a fingerstyle arrangement based on guitar patterns in *Devil's got you beat*. This example picks up from there, regenrating the exact same guitar tab, but this time properly acknowledging the two guitar voices: the part played by with the thumb and the part played with the other fingers. The setup is the same as before.
...
This changes the phrases seen earlier as they must be split into two complementary phrases. Voice ID is indicated in `track`. There are now two tracks, but they are represented by different voices, not by different music staves. Therfore, the two tracks will use `voice = 1` and `voice = 2`, respectively, but they will not have different `tabstaff` IDs.
...
Notice the 1 and 2 passed to the respective calls to `track` for the voice ID. Subsequently, in the call to `trackbind` the two tracks are assigned the same staff ID. By default, not providing `tabstaff` leads to a unique staves per track. This is the more common usage. Providing non-unique staff IDs will throw an error if the voice IDs for a single staff ID are not unique.

## Result

This is a somewhat frustrating refactor for such a small change in result, but keep in mind it is just an comparative example for illustration. Normally, it would only have been coded this second way to begin with. The new output is shown. It is almost identical, as it should be, with the single exception of a change in stem direction representing the two voices. Note that the order in which the tracks are bound determines which voice receives upward stems and which receives downward stems. This is just like when binding multiple tracks destined for their own staves. The actual ID values do not matter; it only matters that they are unique.
