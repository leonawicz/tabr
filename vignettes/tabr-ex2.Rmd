...
```{r voices, eval=FALSE}
# melody 1: voice 1
p1 <- paste(p(pc(pn("s a d4", 3), "s"), "4. 16 16 4. 16 16 8 16 16 4"),
            c(p("s a d s", "8 16 16 4"), p("s a d4 s s", "8 16 16 4 1")))

# melody 1: voice 2
notes <- c(pc(pn("f d c d s", 2), "f g s f d c a2"), "d s c a2 f2 d2", "d s c a2 f2 e2 d2")
...
t1 <- pc(pct(p1[1], 3), p3, p1[1], p1[2]) %>% track(tuning, 1, ms_transpose = 1, ms_key = "e_m")
t2 <- pc(pct(p2[1], 3), p4, p2[2], p2[3]) %>% track(tuning, 2, ms_transpose = 1, ms_key = "e_m")
