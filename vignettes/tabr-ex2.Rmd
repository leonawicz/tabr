...
This can be done by specifying `ms_transpose = 1` and `key = e_` in `track`. This transposed the *music staff* up by one semitone. `key` is necessary for ensuring the staff is written as in the key of E flat rather than D sharp following transposition. Note that the call to `tab` still specifies `key = "dm"` since this is the overall perspective and presentation of the guitar tab.
...
trackbind(t1, t2, tabstaff = c(1, 1)) %>% score() %>% lilypond("out.ly", "dm", "4/4", "4 = 115", header)
t1 <- glue(pct(p1[1], 3), p3, p1[1], p1[2]) %>% track(tuning, 1, ms_transpose = 1, ms_key = "e_")
t2 <- glue(pct(p2[1], 3), p4, p2[2], p2[3]) %>% track(tuning, 2, ms_transpose = 1, ms_key = "e_")
trackbind(t1, t2, tabstaff = c(1, 1)) %>% score() %>% lilypond("out2.ly", "dm", "4/4", "4 = 115", header)
```
