...
```{r music21}
m21 <- "4/4 CCC#4.. trip{c#8 d'- e-' f g a' b'} D4~# D E F r B16 trip{c4 d e}"
x <- from_music21(m21)
x
...
y <- to_tabr("chorrrds", chords = chords)
identical(x, y)
```
...
Note that this last step requires a LilyPond installation. This vignette is part of the music programming and music data analysis collection, but for completeness, the example below shows what you can do on the transcription side.
