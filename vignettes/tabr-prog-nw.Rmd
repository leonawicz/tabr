...
There is no need to attach a special class to a character string to explicitly differentiate noteworthy strings from ordinary strings in R. It is fine to work with strings as usual. However, it is important to be familiar with the requirements of a noteworthy string so that you can construct them properly.

Functions in `tabr` that take noteworthy strings as input perform internal checks of noteworthiness for you and will throw an error if you provide an unworthy string. These checks are skipped if the input already has the `noteworthy` class. Otherwise, if it is noteworthy, the class is added. If a function returns a noteworthy string, the class will also be `noteworthy`.

The class is mostly used internally by a variety of functions to avoid needless repeated checks of noteworthiness, but otherwise this is an ephemeral class that can quickly be lost when performing your own string manipulations. Users do not need to be directly concerned with the class. However, it is important to understand that the notion of noteworthy strings is implemented throughout `tabr` for the purpose of thorough and strict input validation. This leads to more robust function behavior and consistent user programming experience by rejecting problematic string input early.
...
This reports whether the entire string conforms to all requirements for it to be valid `tabr` syntax. `is_note` and `is_chord` provide more detailed information on the space-delimited entries in a string. They return a logical vector reporting whether each entry contains a valid note or valid chord representation, respectively.

Notice how the vectorized results accounts for the expansion operator in `b2*2`.
