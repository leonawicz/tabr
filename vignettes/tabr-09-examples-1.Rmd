---
title: "Short script example"
output: rmarkdown::html_vignette
vignette: >
  %\VignetteIndexEntry{Short script example}
...
This is example is more complex. It does not represent a complete tab of a song; it is just a short arrangement of two basic fingerpicking patterns similar to but not exactly the same as heard in Blues Saraceno's *Devil's got you beat*. While not a complete tab, it is much more thorough than the opening example, making use of several techniques and functions that have been covered.
...
song <- glue(pct(glue(p1[[2]], e1), 3), volta(pct(p2, 3), 1), p1[[1]], e1, p1[[1]], e2) %>%
  track(tuning) %>% score() %>% tab(song, "out.pdf", "dm", "4/4", "4 = 115", header)
```
...
The slighest change forces you to split things up and expand the code, just like in a tab. And most music, at least the kind for which there is any point in transcribing, is going to change things up often even for repeated sections.
