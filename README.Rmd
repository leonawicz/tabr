...
An important piece of syntax is the octave. In conjunction with a note, specifying a unique pitch requires the octave number, either in tick format (recommended; comma and single quote, e.g., `c,`, `c`, `c'`) or integer format (not recommended, more limited utility, e.g., `c2`, `c`, `c4`). Octave 3 is the implicit default; there is no tick and explicitly adding the `3` is unnecessary. The pitches `c d e f g a b` (default octave) are the notes in the octave below middle C (`c'`).
