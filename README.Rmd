...
There is also a `noteinfo` class and a `music` class. You can learn more in the vignettes. The `music` class is particularly useful for efficient music data entry and basic transcription tasks.
...
The third argument, `string` only applies to fretted string instruments and is always optional. In this example it specifies the strings of a guitar. Providing this information in conjunction with the pitch fixes the frets so that LilyPond does not have to guess them. Note that the `x` shown below is just a placeholder indicating no need to specify a string for the quarter note rest. You can put a string number there but it is ignored.
